...
class dsLoader():
    ...
    def get_verbdic(self):
            with open(iot.get_wd()+'/data/gaze/verb_idx.txt') as theFile:
                verbs = theFile.readlines()
            verb_dic = {}
            for v in verbs:
                v = v.replace('\n', '')
                v_id = int(v.replace('/', '-').split(' ')[-1])
                vname = '-'.join(v.replace('/', '-').split(' ')[:-1]).lower()
                # verb_dic[int(v.split(' ')[-1])]='_'.join(v.split(' ')[0:-1])
                verb_dic[v_id] = vname
            return verb_dic
    ...
    def load_hdf(self,hdf_prot_file):
        with h5py.File(hdf_prot_file, "r") as f:
            ...
            for seq_ky in seq_lst:
                ...
                aname = f[seq_ky]['aname'].value
                if aname in self.train_classes:
        ...
        print('Train Actions: ',[aname+': '+str(len(self.train_action_samples[aname])) for aname in self.train_action_samples])
...
if __name__ == '__main__':
    dsl=dsLoader(iot.get_wd()+'/data/gaze/cnt-999_c3d_features.hdf5')
    dsl.kshot_sample_set(kshot_seed=1)
    dsl.get_batch(0, 0, stream_mode=0)
